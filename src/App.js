...
const App = () => {
  ...
  return <div className="appShell" id="top">
    <header className="siteHeader"><div className="headerContent">
      <a className="brand" href="#top"><img src={`${process.env.PUBLIC_URL}/logo.png`} alt="" /><span><small>LEARNING TOOL</small><strong>Quiz Application</strong></span></a>
      <a className="headerLink" href="https://github.com/a2rp/quiz-application" target="_blank" rel="noopener noreferrer"><FiGithub /> Source</a>
    </div></header>
    <main className="pageContent"><QuizApplication /></main>
    <footer className="siteFooter">
      ...
      <div className="footerLinks" aria-label="Social and support links">{footerLinks.map(([label, href, Icon]) => <a key={label} href={href} aria-label={label} title={label} target="_blank" rel="noopener noreferrer"><Icon /></a>)}</div>
    </footer>
